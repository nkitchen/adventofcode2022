...
def main():
    ...
    x_min = min(path[:, 0].min() for path in paths)
    x_max = max(path[:, 0].max() for path in paths)
    y_min = 0
    y_max = max(path[:, 1].max() for path in paths)
    a = x_min - 1
    b = 0
    ...
    while True:
        ...
        if (x not in range(x_min, x_max + 1) or
            y not in range(y_min, y_max + 1)):
            break
